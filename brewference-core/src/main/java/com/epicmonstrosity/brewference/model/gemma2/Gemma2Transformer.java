...
public class Gemma2Transformer implements TransformerGraph {
    private final Config config;
    private final AttentionEngine attentionEngine;
    private final AttentionPattern attentionPattern;

    public Gemma2Transformer(final Config config,
    ...
    public void forward(final int token, final int pos, final RunState runState, final QuantizedWeights weights) {
        ...
        for (int layer = 0; layer < config.getNumLayers(); layer++) {
            ...
            // Rope
            NonInterleavedRope.apply(runState, config, layerContext,
                    () -> attentionPattern.ropeFrequencyBase(layerContext),
                    null);
