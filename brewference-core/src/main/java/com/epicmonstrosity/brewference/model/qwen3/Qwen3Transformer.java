...
public class Qwen3Transformer implements TransformerGraph {
    private final Config config;
    private final AttentionEngine attentionEngine;

    public Qwen3Transformer(final Config config,
    ...
    public void forward(final int token, final int pos, final RunState runState, final QuantizedWeights weights) {
        ...
        final int qWidth = config.getQueryAttentionWidth();
        final int kvWidth = config.getKeyValueDim();
        ...
        for (int layer = 0; layer < config.getNumLayers(); layer++) {
            final LayerContext layerContext = new LayerContext(pos, layer, layer * config.getMaxSequenceLength() * config.getKeyValueDim(), runState.promptSize);
            ...
            // QKV Projections
            Linear.matmul(runState.q, runState.xb, weights.wq, layer * dim * qWidth, dim, qWidth);
            Linear.matmul(runState.k, runState.xb, weights.wk, layer * dim * kvWidth, dim, kvWidth);
            Linear.matmul(runState.v, runState.xb, weights.wv, layer * dim * kvWidth, dim, kvWidth);
            ...
            // Rope
            NonInterleavedRope.apply(runState, config, layerContext, () -> 1_000_000.0f, null);
            ...
            Linear.matmul(runState.xb2, runState.xb, weights.wo, layer * dim * qWidth, qWidth, dim);
